...
- Provider onboarding for OpenAI, OpenRouter, Groq, Ollama, LM Studio, custom OpenAI-compatible remotes, Anthropic, Gemini, and Replicate/fal media with encrypted key storage and model capability metadata; account-runtime onboarding for Codex and Claude Code through their installed CLIs, with per-chat native session persistence, stale Claude session-process recovery, Milim approval modes, and visible tool events.
