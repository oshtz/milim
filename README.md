...
- Workspace and Git panels with guarded fetch/diff/commit/sync actions, local branch switching and creation, model-generated commit subjects when the message is left blank, turn-level Git worktree checkpoints with restore actions, a shared side panel that switches between artifacts, browser, and Git, a top-right Context popover for compact workspace, active plan/goal, browser, model, and source state, filesystem/shell tools rooted to a selected folder, generated-file diff review/apply/save-to-workspace flows for selected artifact revisions, Docker sandbox runs, automatic preview-side-panel DOM tools scoped to the active inspectable preview surface, optional OS-level computer-use screen/input control, and Review/Guarded/Open tool approval modes.
