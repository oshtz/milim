...
- Model-agnostic Agents store identity, system prompt, tools, and skills while interactive runs use the thread-selected model. Child agents inherit the parent run model unless explicitly overridden. Schedules own a required model plus attached file context and visible result threads; legacy schedules can fall back once to their saved Agent model until edited.
...
- Searchable settings with section status indicators, themes with contrast validation for custom palettes, custom style settings, configurable app-window keyboard shortcuts (Previous thread defaults to `Ctrl+Tab` on Windows/macOS and opens a compact recent-thread switcher), Ctrl/Cmd `+` and `-` UI scaling, resizable sidebar/inspector dividers with double-click or Enter reset, frameless window controls, close-to-tray background mode, persisted window state, shared app notices for background events, and startup plus background GitHub Release update checks for Windows/macOS artifacts with checksum verification and a top-bar one-click install action.
