...
const persistedSessions = JSON.stringify({
  state: {
    ...
    sidebar: { sectionOrder: ["chats", "project:C:\\keep"], projectFolders: ["C:\\keep"] },
  },
  version: 0,
});
...
const { purgeExpiredArchivesAfterHydration, useSessions } = await import("../src/sessions/store.js");
const { flushDeferredUserStateWrites } = await import("../src/persistence/userStateStorage.js");

purgeExpiredArchivesAfterHydration();
await new Promise((resolve) => setTimeout(resolve, 0));

assert(
  !calls.some((call) => call.command === "user_state_set" && call.args?.key === "milim.sessions"),
...
assert(stored.state.sessions[0]?.id === "persisted-session", "startup archive cleanup should preserve hydrated sessions");
assert(stored.state.sessions[1]?.id === "title-only-session", "hydration should keep recovered title-only sessions");
assert(Array.isArray(stored.state.sessions[1]?.messages), "title-only sessions should hydrate with an empty messages array");
assert(stored.state.projects[0]?.folder === "C:\\keep", "startup archive cleanup should preserve hydrated projects");
assert(stored.state.sessions[0]?.messages[0]?.content === "keep me", "startup archive cleanup should preserve messages");
