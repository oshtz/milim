...
function sanitizeThemeId(value: string): string {
  try {
    const parsed = JSON.parse(value);
    return typeof parsed === "string" ? JSON.stringify(parsed) : JSON.stringify(String(parsed ?? ""));
  } catch {
...
function sanitizeJsonString(value: string): string {
  try {
    const parsed = JSON.parse(value);
    return JSON.stringify(typeof parsed === "string" ? parsed : String(parsed ?? ""));
  } catch {
    return JSON.stringify(value);
  }
}
...
function sanitizeUserStateValue(key: string, value: string | null): string | null {
...
async function invokeUserState<T>(command: string, args?: Record<string, unknown>): Promise<T> {
  const testInvoke = (globalThis as TestGlobal).__MILIM_TEST_INVOKE__;
  if (testInvoke) return testInvoke<T>(command, args);
  return invoke<T>(command, args);
}

function stateGetCommand(key: string): { command: string; args?: Record<string, unknown> } {
  return key === SESSIONS_KEY ? { command: "user_sessions_get" } : { command: "user_state_get", args: { key } };
}

function stateSetCommand(key: string, value: string): { command: string; args?: Record<string, unknown> } {
  return key === SESSIONS_KEY ? { command: "user_sessions_set", args: { value } } : { command: "user_state_set", args: { key, value } };
}

function stateDeleteCommand(key: string): { command: string; args?: Record<string, unknown> } {
  return key === SESSIONS_KEY ? { command: "user_sessions_delete" } : { command: "user_state_delete", args: { key } };
}
...
async function flushDeferredWrite(key: string): Promise<void> {
  ...
  try {
    ...
    if (lastWrittenValues.get(key) === entry.value) {
      if (hadPrevious && previousValue !== undefined) lastWrittenValues.set(key, previousValue);
...
function deferUserStateWrite(key: string, value: string): Promise<void> {
  ...
  if (!entry) {
    entry = { value, timer: null, resolve: [], reject: [] };
  ...
  if (entry.timer != null) clearTimeout(entry.timer);
  entry.timer = setTimeout(() => void flushDeferredWrite(key), DEFERRED_WRITE_DELAY_MS);
  return promise;
}
...
export async function flushDeferredUserStateWrites(key?: UserStateKey): Promise<void> {
  ...
  await Promise.all(Array.from(deferredWrites.keys()).map((pendingKey) => flushDeferredWrite(pendingKey)));
}
...
export function importLegacyLocalStorageOnce(): Promise<void> {
  ...
  if (!legacyImportPromise) {
    legacyImportPromise = invokeUserState<void>("user_state_import_legacy", { entries: legacyEntries() }).catch((error) => {
...
export function readUserStateKey(key: UserStateKey): string | null | Promise<string | null> {
  if (!inTauri()) {
  ...
  return importLegacyLocalStorageOnce().then(async () => {
    ...
    const value = await invokeUserState<string | null>(command.command, command.args);
...
export function writeUserStateKey(key: UserStateKey, value: string): void | Promise<void> {
  ...
  const pending = deferredWrites.get(key);
  if (pending?.value === sanitized || lastWrittenValues.get(key) === sanitized) return undefined;
  incrementPerfCounter(`persist.${key}.write`);
  recordPerfMeasure(`persist.${key}.bytes`, sanitized.length);
