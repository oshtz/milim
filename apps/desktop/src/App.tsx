...
import { shouldCheckOnboardingModels, shouldShowOnboarding, useOnboarding } from "./onboarding/store";
import { importLegacyLocalStorageOnce } from "./persistence/userStateStorage.js";
import { purgeExpiredArchivesAfterHydration, useSessions } from "./sessions/store";
...
const ThemePicker = lazy(() => import("./components/ThemePicker").then((mod) => ({ default: mod.ThemePicker })));
const AgentsManager = lazy(() => import("./components/AgentsManager").then((mod) => ({ default: mod.AgentsManager })));
const SkillsManager = lazy(() => import("./components/SkillsManager").then((mod) => ({ default: mod.SkillsManager })));
const SchedulesManager = lazy(() => import("./components/SchedulesManager").then((mod) => ({ default: mod.SchedulesManager })));
...
function OnboardingGate() {
  ...
  const developerShowOnboarding = useOnboarding((s) => s.developerShowOnboarding);
  const dismissedAt = useOnboarding((s) => s.dismissedAt);
  const shouldCheckModels = shouldCheckOnboardingModels(status, developerShowOnboarding);
  ...
  if (!hydrated || (shouldCheckModels && !modelsChecked)) return <div className="onboarding-preflight" data-testid="onboarding-preflight" />;
  if (!shouldShowOnboarding(status, modelsReady, developerShowOnboarding, dismissedAt)) return null;
...
function AppContent() {
  ...
  useEffect(() => {
    if (!inTauri) return;
    try {
      void getCurrentWebview().setZoom(uiSize / 100).catch(() => {});
    } catch {
      /* not in tauri */
    }
  }, [uiSize]);
  ...
  return (
    <div className={appClassName} onContextMenu={openAppContextMenu}>
      ...
      <Suspense fallback={null}>
        ...
        {schedulesOpen && showSchedules && <SchedulesManager onClose={() => setSchedulesOpen(false)} />}
      </Suspense>
      <AutoUpdater />
      <ResizeHandles />
    </div>
