...
const COMMIT_MESSAGE_SYSTEM_PROMPT = "Write one professional Git commit subject. Return exactly one line, no markdown, no quotes. Use imperative mood when natural. Keep it under 72 characters.";
...
function changeTone(status: WorkspaceGitStatus): "clean" | "dirty" | "conflict" {
...
function branchMetaLabel(branch: WorkspaceGitBranch): string {
  const sync = branch.ahead || branch.behind ? ` · ${branch.ahead} ahead / ${branch.behind} behind` : "";
...
function remoteWebUrl(remote: string | null): string | null {
  ...
  const scp = trimmed.match(/^(?:[^@\\/\s]+@)?([^:\\/\s]+(?:\.[^:\\/\s]+)+):([^\\\s]+\/[^\\\s]+)$/);
...
function gitTaskPrompt(folder: string, status: WorkspaceGitStatus, task: string, instruction: string): string {
  return [
    task,
    `Folder: ${folder}`,
    `Branch: ${branchLabel(status)}`,
...
function syncCommand(status: WorkspaceGitStatus): { action: WorkspaceGitAction; label: string; disabled?: boolean; title: string } | null {
  if (status.behind) {
    return status.has_changes
      ? { action: "pull", label: "Pull", disabled: true, title: "Clean the worktree before pulling." }
      : { action: "pull", label: "Pull", title: "Run git pull --ff-only." };
  }
  if (status.ahead && status.upstream) return { action: "push", label: "Push", title: "Run git push." };
  if (!status.upstream && status.remote && status.branch) {
    return { action: "publish", label: "Publish", title: `Run git push -u origin ${status.branch}.` };
  }
  return null;
}

function actionOutputText(result: WorkspaceGitActionResult): string {
  return [result.stdout, result.stderr].map((part) => part.trim()).filter(Boolean).join("\n\n");
}
...
function commandPreview(action: WorkspaceGitAction, status: WorkspaceGitStatus, message: string, stageAll: boolean): string {
  if (action === "commit") {
    return `${stageAll ? "git add -A && " : ""}git commit -m ${quoteArg(message)}`;
  }
  if (action === "commit_push") {
    const push = status.upstream ? "git push" : `git push -u origin ${status.branch || "<branch>"}`;
  ...
  if (action === "publish") return `git push -u origin ${status.branch || "<branch>"}`;
...
function changedSegments(text: string, other: string | null): { text: string; changed: boolean }[] {
  ...
  while (end < maxEnd && text[text.length - 1 - end] === other[other.length - 1 - end]) end += 1;
...
function diffTextSegments(row: DiffRow, index: number, rows: DiffRow[]): { text: string; changed: boolean }[] {
  if (row.kind === "add") {
    const previous = rows[index - 1];
    return changedSegments(row.text, previous?.kind === "delete" ? previous.text : null);
  }
  if (row.kind === "delete") {
...
function renderDiffText(row: DiffRow, index: number, rows: DiffRow[], query: string): ReactNode {
  ...
  return diffTextSegments(row, index, rows).flatMap((segment) => {
    ...
    if (!matcher) return [<span className={className} key={key++}>{segment.text}</span>];
    return segment.text.split(matcher).filter(Boolean).map((part) => {
      const isMatch = part.toLowerCase() === term.toLowerCase();
      return isMatch
        ? <mark className={className} key={key++}>{part}</mark>
        : <span className={className} key={key++}>{part}</span>;
    });
  });
}
...
function commitMessageContext(status: WorkspaceGitStatus, diff: string): string {
  return [
    `Branch: ${branchLabel(status)}`,
...
async function generateCommitMessage(model: string, status: WorkspaceGitStatus, diff: string): Promise<string> {
...
async function generateAccountRuntimeCommitMessage(preferredModel: string, folder: string, status: WorkspaceGitStatus, diff: string): Promise<string | null> {
  ...
  const message = cleanGeneratedCommitMessage(response);
  if (!message) throw new Error(`${codexModel ? "Codex" : "Claude Code"} returned an empty commit message.`);
  return message;
}

async function generateCommitMessageWithFallback(preferredModel: string, folder: string, status: WorkspaceGitStatus, diff: string): Promise<string> {
  let lastError = "";
  try {
    const accountRuntimeMessage = await generateAccountRuntimeCommitMessage(preferredModel, folder, status, diff);
    if (accountRuntimeMessage) return accountRuntimeMessage;
  } catch (error) {
    lastError = error instanceof Error ? error.message : "account runtime generation failed";
  }

  const candidates = commitMessageModelCandidates(await listModelsDetailed(), preferredModel);
  if (!candidates.length) {
    throw new Error(lastError || "Enter a commit message, or add a reachable chat model to generate one.");
  }
...
export function GitPanel({
  ...
  const [commandBusy, setCommandBusy] = useState<WorkspaceGitAction | null>(null);
  const [commandResult, setCommandResult] = useState<WorkspaceGitActionResult | null>(null);
  const [diffResult, setDiffResult] = useState<WorkspaceGitActionResult | null>(null);
  const [commandMenu, setCommandMenu] = useState<WorkspaceGitAction | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      setStatus((previous) => next ?? (statusFolder === selectedFolder ? previous : null));
  ...
  useEffect(() => {
    ...
    function refreshOnFocus() {
      const timestamp = Date.now();
      if (!shouldRefreshGitStatus(lastGitStatusRunAtRef.current, timestamp)) return;
  ...
  const SyncIcon = sync?.action === "pull" ? Download : sync?.action === "push" ? ArrowUp : GitBranch;
  ...
  const renderedDiffSections = renderedDiffRows.length ? diffSections(renderedDiffRows) : [];
  const renderedDiffStats = renderedDiffRows.length ? diffStats(renderedDiffRows) : { files: 0, additions: 0, deletions: 0 };
  const normalizedDiffSearch = diffSearch.trim().toLowerCase();
  const visibleDiffRows: { row: DiffRow; index: number; section: DiffSection | null; sectionIndex: number }[] = [];
  let currentSectionIndex = -1;
  for (let index = 0; index < renderedDiffRows.length; index += 1) {
    if (renderedDiffSections[currentSectionIndex + 1]?.start === index) currentSectionIndex += 1;
    const section = currentSectionIndex >= 0 ? renderedDiffSections[currentSectionIndex] : null;
    if (section && collapsedDiffSections.has(section.id) && renderedDiffRows[index].kind !== "file") continue;
    visibleDiffRows.push({ row: renderedDiffRows[index], index, section, sectionIndex: currentSectionIndex });
  }
  const diffSearchMatches = normalizedDiffSearch
    ? visibleDiffRows.filter(({ row }) => row.text.toLowerCase().includes(normalizedDiffSearch)).map(({ index }) => index)
    : [];
  ...
  const filteredBranches = normalizedBranchFilter
    ? branches.filter((branch) => branch.name.toLowerCase().includes(normalizedBranchFilter) || branch.upstream?.toLowerCase().includes(normalizedBranchFilter))
    : branches;
  const nextBranchName = newBranchName.trim();
  const canCreateBranch = Boolean(nextBranchName) && !branches.some((branch) => branch.name === nextBranchName);
  ...
  async function openFolder() {
    await openArtifactLocation(readyStatus.root || selectedFolder, "folder").catch(() => undefined);
  }
  ...
  function stepDiffSearch(delta: number) {
    ...
    setDiffSearchIndex((activeDiffSearchIndex + delta + diffSearchMatches.length) % diffSearchMatches.length);
  }
    ...
    return (
      <div className="git-diff-toolbar">
        ...
        <label className="git-diff-search">
          ...
          />
          <span>{diffSearchMatches.length ? `${activeDiffSearchIndex + 1}/${diffSearchMatches.length}` : "0/0"}</span>
          <button type="button" disabled={!diffSearchMatches.length} onClick={() => stepDiffSearch(-1)}>Prev</button>
          <button type="button" disabled={!diffSearchMatches.length} onClick={() => stepDiffSearch(1)}>Next</button>
        </label>
      </div>
    );
  }
  ...
  function renderDiffView() {
    return (
      <div className="git-diff-view" role="table" aria-label="Unified diff" ref={diffViewRef}>
        {visibleDiffRows.map(({ row, index, section, sectionIndex }) => {
          const collapsed = section ? collapsedDiffSections.has(section.id) : false;
          const matchIndex = diffSearchMatchIndexByRow.get(index);
          const searchActive = matchIndex === activeDiffSearchIndex && diffSearchMatches.length > 0;
          return (
            <div
              className={`git-diff-row ${row.kind}${searchActive ? " search-active" : ""}`}
              role="row"
              key={`${index}:${row.kind}:${row.oldNo}:${row.newNo}`}
              data-section-index={row.kind === "file" ? sectionIndex : undefined}
              data-match-index={matchIndex}
            >
              <span className="git-diff-gutter old" aria-label={row.oldNo ? `Old line ${row.oldNo}` : undefined}>{row.oldNo}</span>
              <span className="git-diff-gutter new" aria-label={row.newNo ? `New line ${row.newNo}` : undefined}>{row.newNo}</span>
              <span className="git-diff-marker" aria-hidden="true">{row.marker}</span>
              {row.kind === "file" && section ? (
                <button className="git-diff-file-toggle" type="button" onClick={() => toggleDiffSection(section.id)}>
                  <span>{row.text}</span>
                  <small>+{section.additions}/-{section.deletions} ֲ· {section.lineCount} lines ֲ· {collapsed ? "Expand" : "Collapse"}</small>
                </button>
              ) : (
                <code>{renderDiffText(row, index, renderedDiffRows, diffSearch)}</code>
              )}
            </div>
          );
        })}
  ...
  function renderDiffContent(inline = false) {
    if (!visibleDiffRows.length) return <div className="git-diff-empty">No diff output.</div>;
    if (inline) {
      return (
        <>
          {renderDiffToolbar()}
  ...
  async function runGitCommand(action: WorkspaceGitAction, options: { message?: string; stage_all?: boolean; branch?: string } = {}) {
  ...
  async function runCommit(push: boolean) {
    ...
    if (!message) {
      ...
      setNotice(null);
      try {
        const diff = await runWorkspaceGitAction("diff", { staged_only: !stageAll });
        const output = actionOutputText(diff);
        if (!output.trim()) {
          setNotice("No diff available for a commit message.");
          return;
        }
        message = await generateCommitMessageWithFallback(model, selectedFolder, readyStatus, output);
        setCommitMessage(message);
      } catch (error) {
        setNotice(error instanceof Error ? error.message : "Couldn't generate a commit message.");
        return;
      } finally {
        setGeneratingCommitMessage(false);
      }
    }
    void runGitCommand(push ? "commit_push" : "commit", { message, stage_all: stageAll });
  }

  if (!expanded) {
    return (
      <section className={`git-panel git-panel-collapsed${onOpenPanel ? " git-panel-launcher" : ""}`} data-testid="git-panel" aria-label="Git environment">
        <div className="git-panel-collapsed-row">
          ...
          >
            ...
            <span className={`git-panel-toggle-stat ${changeTone(readyStatus)}`}>{compactChangeLabel(readyStatus)}</span>
  ...
  return (
    ...
    <section className={`git-panel${forceExpanded ? " git-panel-workspace" : ""}`} data-testid="git-panel" aria-label="Git environment">
      <div className="git-panel-head">
        ...
        >
          ...
          <span className="git-panel-title-copy">
            <strong title={readyStatus.root || selectedFolder}>{repoName}</strong>
          </span>
      ...
      <div className="git-panel-subhead">
        <button
          ...
          type="button"
          data-testid={forceExpanded ? "git-branch-selector" : "git-copy-branch"}
          onClick={() => {
            if (!forceExpanded) {
              void copyBranch();
              return;
      ...
      <div className={`git-summary ${changeTone(readyStatus)}`} data-testid="git-changes-row">
      ...
      {filePreview.length > 0 && (
        <div className="git-file-list" aria-label="Changed files">
          {filePreview.map((change) => (
            <div className="git-file-row" key={`${change.status}:${change.path}`} title={changedFileTitle(change)}>
              <span className="git-file-status">{gitStatusLabel(change.status)}</span>
              <span className="git-file-path">{compactPath(cleanGitPath(change.path))}</span>
            </div>
          ))}
          {moreFiles > 0 && <div className="git-file-more">+{moreFiles} more</div>}
        </div>
      )}

      <div className="git-panel-details">
        <button className="git-row" type="button" onClick={openFolder} title={readyStatus.root || selectedFolder}>
          <Folder size={14} />
          <span className="git-row-label">Local</span>
          <span className="git-row-value">{compactPath(readyStatus.root || selectedFolder)}</span>
        </button>

        {remoteUrl ? (
          <button className="git-row" type="button" onClick={openRemote} title={remoteUrl}>
            <GitRemote size={14} />
            <span className="git-row-label">Remote</span>
            <span className="git-row-value">{syncLabel(readyStatus)}</span>
          </button>
        ) : (
          <div className="git-row git-row-static" title={readyStatus.upstream || readyStatus.remote || undefined}>
            <GitRemote size={14} />
            <span className="git-row-label">Remote</span>
            <span className="git-row-value">{syncLabel(readyStatus)}</span>
          </div>
      ...
      <div className="git-panel-actions git-command-actions" aria-label="Git commands">
        {(!forceExpanded || readyStatus.has_changes) && (
          <button
            className="git-action"
            type="button"
            data-testid="git-action-diff"
            title={readyStatus.has_changes ? "Open diff panel" : "No changes to diff"}
        ...
        <button
          ...
          title={readyStatus.remote ? "Run git fetch --prune" : "No remote configured"}
        ...
        {(!forceExpanded || readyStatus.has_changes) && (
          <button
            ...
            title={readyStatus.conflicts ? "Resolve conflicts before committing" : readyStatus.has_changes ? "Open commit commands" : "No changes to commit"}
        ...
        {sync && (
          ...
          >
            <SyncIcon size={13} />
            <span>{commandBusy === sync.action ? `${sync.label}...` : sync.label}</span>
          </button>
        )}
        ...
        {forceExpanded && commandResult && (
          <div
            ...
            role="status"
            title={commandOutput || commandResult.command || commandResult.message}
          >
      ...
      <button className="git-agent-action" type="button" onClick={() => onDraftAction(agentReviewPrompt(selectedFolder, readyStatus))}>
        <Lightbulb size={13} />
      ...
      {forceExpanded && diffResult && (
        <section className={`git-workspace-review ${diffResult.ok ? "" : "error"}`} aria-label="Git diff">
          <div className="git-modal-head">
            <span>
              <Code size={13} />
              <strong>{diffResult.message}</strong>
            </span>
            <button type="button" className="git-icon-btn" title="Close diff" aria-label="Close diff" onClick={() => setDiffResult(null)}>
              <X size={13} />
            </button>
          </div>
          {diffResult.command && <code className="git-command-preview">{diffResult.command}</code>}
          {renderDiffContent(true)}
        </section>
      )}
    </section>
    {typeof document !== "undefined" && branchMenuOpen && createPortal(
      <div className="git-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setBranchMenuOpen(false)}>
        <section className="git-modal git-branch-modal" role="dialog" aria-modal="true" aria-label="Switch branch">
          <div className="git-modal-head">
            <span>
              <GitBranch size={13} />
              <strong>Branch</strong>
            </span>
            <button type="button" className="git-icon-btn" title="Close" aria-label="Close" onClick={() => setBranchMenuOpen(false)}>
              <X size={13} />
            </button>
          </div>
          <label className="git-branch-search">
            <Search size={12} />
            <input value={branchFilter} placeholder="Search branches..." onChange={(event) => setBranchFilter(event.currentTarget.value)} />
          </label>
          <div className="git-branch-list" aria-label="Local branches">
            {filteredBranches.length ? filteredBranches.map((branch) => (
              <button
                ...
                onClick={() => void runGitCommand("checkout_branch", { branch: branch.name })}
                title={branch.name}
              >
                <span>
                  <GitBranch size={12} />
          ...
          <div className="git-branch-create">
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter" && canCreateBranch) void runGitCommand("create_branch", { branch: nextBranchName });
              }}
            />
            <button type="button" disabled={!canCreateBranch || Boolean(commandBusy)} onClick={() => void runGitCommand("create_branch", { branch: nextBranchName })}>
              <GitBranch size={12} />
              <span>{commandBusy === "create_branch" ? "Creating..." : "Create"}</span>
            </button>
          </div>
          {notice && (
    ...
    {typeof document !== "undefined" && commandMenu && createPortal(
      <div className="git-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setCommandMenu(null)}>
        <section className="git-modal git-command-modal" role="dialog" aria-modal="true" aria-label={`${actionLabel(commandMenu)} command`}>
          <div className="git-modal-head">
            <span>
              <GitBranch size={13} />
              <strong>{actionLabel(commandMenu)}</strong>
            </span>
            <button type="button" className="git-icon-btn" title="Close" aria-label="Close" onClick={() => setCommandMenu(null)}>
              <X size={13} />
            </button>
          </div>
            ...
            <>
              ...
              <label className="git-check-row">
                <input
                  ...
                  checked={stageAll}
                  disabled={Boolean(commandBusy) || generatingCommitMessage}
                  onChange={(event) => setStageAll(event.currentTarget.checked)}
                />
                <span>Include unstaged changes</span>
              </label>
              <code className="git-command-preview">{commandPreview("commit", readyStatus, commitMessage, stageAll)}</code>
              {notice && (
                <div className="git-command-modal-note" role="status">
                  {notice}
                </div>
              ...
              <div className="git-command-options">
                <button
                  type="button"
                  disabled={!commitReady || Boolean(commandBusy) || generatingCommitMessage}
                  onClick={() => void runCommit(false)}
                >
                  <GitCommit size={13} />
                  <span>{generatingCommitMessage ? "Generating..." : commandBusy === "commit" ? "Committing..." : "Commit"}</span>
                </button>
                <button
                  type="button"
                  disabled={!commitReady || !canCommitPush || Boolean(commandBusy) || generatingCommitMessage}
                  title={canCommitPush ? "Commit, then push" : "Remote branch required and not behind"}
                  onClick={() => void runCommit(true)}
                >
                  <ArrowUp size={13} />
                  <span>{generatingCommitMessage ? "Generating..." : commandBusy === "commit_push" ? "Pushing..." : "Commit and push"}</span>
                </button>
              </div>
            </>
          ) : (
            <>
              <code className="git-command-preview">{commandPreview(commandMenu, readyStatus, commitMessage, stageAll)}</code>
              <div className="git-command-options">
                <button type="button" disabled={Boolean(commandBusy)} onClick={() => void runGitCommand(commandMenu)}>
                  {commandMenu === "pull" ? <Download size={13} /> : commandMenu === "push" ? <ArrowUp size={13} /> : <GitBranch size={13} />}
                  <span>{commandBusy === commandMenu ? `${actionLabel(commandMenu)}...` : `Run ${actionLabel(commandMenu)}`}</span>
                </button>
              </div>
            </>
          )}
    ...
    {typeof document !== "undefined" && diffResult && !forceExpanded && createPortal(
      <div className="git-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setDiffResult(null)}>
        <section className={`git-modal git-diff-panel ${diffResult.ok ? "" : "error"}`} role="dialog" aria-modal="true" aria-label="Git diff">
          <div className="git-modal-head">
            <span>
              <Code size={13} />
              <strong>{diffResult.message}</strong>
                  ...
                  return (
                    ...
                    >
                      ...
                      {row.kind === "file" && section ? (
                        <button className="git-diff-file-toggle" type="button" onClick={() => toggleDiffSection(section.id)}>
                          <span>{row.text}</span>
                          <small>+{section.additions}/-{section.deletions} · {section.lineCount} lines · {collapsed ? "Expand" : "Collapse"}</small>
                        </button>
                      ) : (
                        <code>{renderDiffText(row, index, renderedDiffRows, diffSearch)}</code>
                      )}
...
export function GitWorkspacePanel({
  ...
  return (
    <aside className={`preview-panel git-workspace-panel${closing ? " closing" : ""}${noEnterMotion ? " no-enter" : ""}`} data-testid="git-workspace-panel" style={style}>
      <div className="preview-toolbar git-workspace-toolbar">
        {modeSwitcher}
        <div className="preview-actions" aria-label="Git panel actions">
          <button className="preview-action" title="Close Git panel" aria-label="Close Git panel" onClick={onClose}>
            <X size={14} />
          </button>
        </div>
      </div>
      <div className="git-workspace-scroll">
        <GitPanel folder={folder} model={model} onDraftAction={onDraftAction} forceExpanded />
      </div>
    </aside>
  );
}
