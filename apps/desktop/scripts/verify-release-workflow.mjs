...
const verifyCommand = 'GITHUB_REF_TYPE=tag GITHUB_REF_NAME="${MILIM_RELEASE_TAG}" pnpm -C apps/desktop verify';
...
for (const needle of [
  ...
  'gh release upload "${MILIM_RELEASE_TAG}" SHA256SUMS.txt --repo "${GITHUB_REPOSITORY}" --clobber',
]) {
