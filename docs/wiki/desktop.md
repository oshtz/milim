...
| Context card | Persisted per thread and grouped into Environment, Task, live Activity, Context, and Sources. The floating card reserves a 300px layout slot that pushes the transcript, can coexist with the inspector on wide layouts, and yields to the inspector below the dual-panel threshold. Live Codex quota is fetched only while the card is open; the adjacent launcher opens the active folder in installed local tools. |
| Inspector | One Preview / Code / Git / Workers surface. Workers owns delegation policy, frozen Ask approval, live flat worker rows, results, activity, stop controls, and isolated worktree diff review/apply. Delegation opens a closed inspector; an already-open non-Workers tab receives a badge without losing focus. |
