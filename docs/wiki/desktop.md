...
| Settings | Settings search returns individual controls, jumps to the matched row, and section navigation shows warning status for incomplete setup. Appearance includes default-off, locally synthesized interface sounds for key controls and visible active-chat results. |
