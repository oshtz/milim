...
| Context card | Persisted per thread and grouped into Workers, Environment, Task, live Activity, Context, and Sources. Sources shows five entries by default with a more/show less toggle. Workers keeps delegation and model settings in a compact disclosure while proposed and running content remains visible; it also owns frozen Ask approval, live rows, results, stop controls, and isolated worktree diff review/apply. The floating card reserves a 300px layout slot that pushes the transcript, can coexist with the inspector on wide layouts, and yields to the inspector below the dual-panel threshold. Live Codex quota is fetched only while the card is open; the adjacent launcher appears only for folder-backed threads and opens the active folder in installed local tools. |
