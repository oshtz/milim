...
| Inspector | One Preview / Code / Git surface for inspecting work products while Worker activity remains visible in Context on wide layouts. |
...
| Keyboard shortcuts | App-window shortcuts are configurable; Previous thread defaults to `Ctrl+Tab` on Windows and macOS, switches to the last viewed thread immediately, and shows a compact recent-thread switcher for repeated presses. Ctrl/Cmd `+` and `-` scale the UI and reveal transient percentage, step, and reset controls in the title bar. The inspector can expand across all space beyond the chat minimum; sidebar and inspector dividers snap closed when dragged 96px past their minimum, reopen when that same drag reverses, and reset on double-click or Enter. |
